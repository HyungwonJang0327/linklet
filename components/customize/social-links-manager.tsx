...
export function SocialLinksManager({ socialLinks, onSocialLinksChange }: SocialLinksManagerProps) {
  ...
  const socialPlatforms = [
    ...
    { id: 'website', name: t('settings.customize.social.website'), icon: '🌐', baseUrl: 'https://', placeholder: 'example.com' }
  ]
  ...
  return (
    <Card className="bg-slate-800/50 border-slate-700/50 backdrop-blur-sm">
      <div className="p-6">
        <h2 className="text-xl font-semibold text-white mb-4">{t('settings.customize.social.title')}</h2>
        <p className="text-slate-400 text-sm mb-6">
          {t('settings.customize.social.description')}
        </p>
        ...
        <div className="space-y-4 mb-6">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    <label className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-slate-400 text-sm">{t('settings.customize.social.enabled')}</span>
                    </label>
                ...
                <div className="space-y-2">
                  <Input
                    label={t('settings.customize.social.url')}
                    value={link.url}
                  ...
                  {link.url && (
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-slate-400">{t('settings.customize.social.preview')}</span>
                      <a
                        href={link.url}
                        target="_blank"
          ...
          {socialLinks.length === 0 && (
            <div className="text-center py-8">
              <div className="text-slate-400 mb-4">{t('settings.customize.social.noLinks')}</div>
              <div className="text-slate-500 text-sm">{t('settings.customize.social.noLinksDesc')}</div>
            </div>
          )}
        </div>
        ...
        <div className="pt-6 border-t border-slate-700/50">
          <h3 className="text-lg font-medium text-white mb-4">{t('settings.customize.social.addNew')}</h3>
          ...
          {socialLinks.length >= socialPlatforms.length && (
            <div className="text-center py-4">
              <div className="text-slate-400 text-sm">{t('settings.customize.social.allAdded')}</div>
            </div>
          )}
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-700/50">
          <h3 className="text-lg font-medium text-white mb-4">{t('settings.customize.social.displayOptions')}</h3>

          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-slate-200 font-medium">{t('settings.customize.social.linkStyle')}</div>
                <div className="text-slate-400 text-sm">{t('settings.customize.social.linkStyleDesc')}</div>
              </div>
              <select className="px-3 py-1 bg-slate-900/50 border border-slate-600 rounded text-white text-sm">
                <option>{t('settings.customize.social.iconOnly')}</option>
                <option>{t('settings.customize.social.iconText')}</option>
                <option>{t('settings.customize.social.buttonStyle')}</option>
              </select>
            </div>

            <div className="flex items-center justify-between">
              <div>
                <div className="text-slate-200 font-medium">{t('settings.customize.social.linkPosition')}</div>
                <div className="text-slate-400 text-sm">{t('settings.customize.social.linkPositionDesc')}</div>
              </div>
              <select className="px-3 py-1 bg-slate-900/50 border border-slate-600 rounded text-white text-sm">
                <option>{t('settings.customize.social.belowProfile')}</option>
                <option>{t('settings.customize.social.pageTop')}</option>
                <option>{t('settings.customize.social.pageBottom')}</option>
              </select>
            </div>
            ...
            <label className="flex items-center gap-3">
              ...
              />
              <div>
                <div className="text-slate-200 font-medium">{t('settings.customize.social.openNewTab')}</div>
                <div className="text-slate-400 text-sm">{t('settings.customize.social.openNewTabDesc')}</div>
              </div>
            </label>
          </div>
        </div>
